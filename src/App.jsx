...
export default function App(){
  ...
  const selectScenario=(key)=>{if(key==="custom"){setAgents([{id:"a1",name:lang==="zh"?"智能体 1":"Agent 1",role:"",provider:"anthropic",model:"claude-sonnet-4-6-20260218",temp:.7,maxTokens:DEFAULT_MAX_TOKENS,color:COLORS[0]}]);setRounds(1);}else{const p=presets[key];setAgents(p.agents.map(a=>({...a})));setRounds(p.rounds);}setPage("setup");};
  const addAgent=()=>{const i=agents.length;setAgents([...agents,{id:`a${Date.now()}`,name:lang==="zh"?`智能体 ${i+1}`:`Agent ${i+1}`,role:"",provider:"anthropic",model:"claude-sonnet-4-6-20260218",temp:.7,maxTokens:DEFAULT_MAX_TOKENS,color:COLORS[i%COLORS.length]}]);};
